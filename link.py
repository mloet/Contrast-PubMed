...
def process_pubmedqa(input):
    ...
    outs, lens = [], []
    for obj in data:
        context = " ".join([c.strip() for c in obj['context']["contexts"] if c.strip()])
        question = obj["question"].strip()
        label = obj["final_decision"].strip()
        assert label in ["yes", "no", "maybe"]
        outs.append({"id": obj['pubid'], "sentence1": question, "sentence2": context, "label": label})
